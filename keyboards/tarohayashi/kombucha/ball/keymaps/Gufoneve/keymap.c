...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
  [BASE] = LAYOUT(
        ...
        LALT(KC_LEFT), MO(LOWER), KC_ENTER,
        KC_MS_BTN1, LT(UPPER, KC_MS_BTN3), LT(LIGHT, KC_SPACE)
    ),
  [LOWER] = LAYOUT(
        XXXXXXX, XXXXXXX,
        LCTL(LWIN(KC_RIGHT)), XXXXXXX, KC_DELETE,
        LCTL(LWIN(KC_LEFT)), XXXXXXX, KC_ESCAPE,
...
const uint16_t PROGMEM encoder_map[][NUM_ENCODERS][2] = {
    ...
    [LOWER] =  { ENCODER_CCW_CW(KC_MS_WH_DOWN, KC_MS_WH_UP)},
    [UPPER] =  { ENCODER_CCW_CW(KC_MS_WH_DOWN, KC_MS_WH_UP)},
...
bool process_record_user(uint16_t keycode, keyrecord_t* record) {
    if (keycode == CPI_I && record->event.pressed) {
        ...
        if(ballconfig.cpi_idx > 5){
            ballconfig.cpi_idx = 4;
        }
